...
import type { DifficultyLevel, ProjectType } from '@/types'
...
const FieldLabel: React.FC<{ children: React.ReactNode; required?: boolean; className?: string }> = ({
  children,
  required,
  className,
}) => (
...
const AddProjectPage: React.FC = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      await projectService.createProject({
        ...
        status: 'pending',
        // Images and zip are handled by multipart upload in a real API
        // screenshots: imageFiles.map(f => f.name),
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminHeader title="Add New Project" subtitle="Create a new platform project" />

      {/* Toolbar */}
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-sora font-bold text-xl text-slate-900">New Project</h2>
          <p className="text-sm text-slate-500 mt-0.5">Fill in the details below to add a project</p>
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
        ...
        <FormSection title="Descriptions">
          <div>
            ...
            <Textarea
              ...
              placeholder="A brief one-to-two sentence summary shown in project cards (20–200 characters)"
              className="min-h-[80px]"
              error={errors.shortDescription?.message}
            />
            <p className="mt-1 text-xs text-slate-400">Used in listing cards and search results.</p>
          </div>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Detailed description of the project — features, architecture, use cases, technical highlights..."
        ...
        <FormSection title="Technologies & Tags">
          ...
          <TagInput
            label="Tags"
            placeholder="Add a tag and press Enter (e.g. AI, Python)"
            tags={tags}
            onChange={setTags}
          />
        </FormSection>
        ...
        <FormSection title="Files & Media">
          <FileDrop
            ...
            description="PNG, JPG, WebP — you can upload multiple images"
            files={imageFiles}
          ...
          <FileDrop
            ...
            description="ZIP archive containing the full project source code"
            files={zipFiles}
