...
const StudentProjectsPage: React.FC = () => {
  ...
  const confirmApprove = () => {
    ...
    setProjects((prev) => prev.map((p) => p.id === approveTarget.id ? { ...p, approvalStatus: 'approved' } : p))
  ...
  const confirmReject = () => {
    ...
    setProjects((prev) => prev.map((p) => p.id === rejectTarget.id ? { ...p, approvalStatus: 'rejected' } : p))
    setRejectTarget(null); setRejectReason(''); setRejectError('')
  }

  const openReject = (project: StudentProject) => { setRejectTarget(project); setRejectReason(''); setRejectError('') }
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminHeader title="Student Projects" subtitle="Manage and moderate student project submissions" />

      <div className="flex items-center">
        <Link to="/admin/dashboard">
          <Button variant="outline" size="sm"><ArrowLeft size={15} />Back to Dashboard</Button>
        </Link>
      </div>
          ...
          <table className="w-full text-sm font-inter">
            <thead>
              <tr className="bg-[#1E3A5F] text-white">
                {['Title','Student Name','Email','Price','Domain','Status','Actions'].map((h, i) => (
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((project) => (
                  <tr key={project.id} className="bg-white hover:bg-slate-50 transition-colors duration-100">
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">{statusBadge(project.approvalStatus)}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-center gap-2">
                        <button title="View details" onClick={() => setViewTarget(project)} className="p-1.5 rounded-lg text-teal-600 bg-teal-50 hover:bg-teal-100 transition-colors duration-150"><Eye size={15} /></button>
                        <button title="Approve project" onClick={() => setApproveTarget(project)} disabled={project.approvalStatus === 'approved'} className="p-1.5 rounded-lg text-green-600 bg-green-50 hover:bg-green-100 transition-colors duration-150 disabled:opacity-40 disabled:pointer-events-none"><Check size={15} /></button>
                        <button title="Reject project" onClick={() => openReject(project)} disabled={project.approvalStatus === 'rejected'} className="p-1.5 rounded-lg text-red-600 bg-red-50 hover:bg-red-100 transition-colors duration-150 disabled:opacity-40 disabled:pointer-events-none"><X size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      <Dialog open={!!viewTarget} onOpenChange={() => setViewTarget(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Submitted student project information</DialogDescription>
          </DialogHeader>
          {viewTarget && (
      ...
      <Dialog open={!!approveTarget} onOpenChange={() => setApproveTarget(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button variant="success" size="sm" onClick={confirmApprove}><Check size={14} />Approve</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={!!rejectTarget} onOpenChange={() => { setRejectTarget(null); setRejectReason(''); setRejectError('') }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => { setRejectTarget(null); setRejectReason(''); setRejectError('') }}>Cancel</Button>
            <Button variant="destructive" size="sm" onClick={confirmReject}><X size={14} />Reject</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
