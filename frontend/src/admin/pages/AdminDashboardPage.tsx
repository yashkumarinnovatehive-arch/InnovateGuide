...
const FALLBACK_STATS: DashboardStats = {
  ...
  pendingApprovals: MOCK_PROJECTS.filter((p) => p.status === 'pending').length,
  adminProjects: MOCK_PROJECTS.filter((p) => p.type === 'admin').length,
  studentProjects: MOCK_PROJECTS.filter((p) => p.type === 'student').length,
  buyEnquiries: 10,
  customEnquiries: 3,
}
...
const AdminDashboardPage: React.FC = () => {
  ...
  const stats: DashboardStats = { ...FALLBACK_STATS }
