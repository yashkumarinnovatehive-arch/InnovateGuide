...
const platformSchema = z.object({
  ...
  whatsappNumber: z
    .string()
    .regex(/^\d{10,15}$/, 'Enter digits only with country code (e.g. 919876543210)')
    .or(z.literal('')),
})
    ...
    <div className="flex flex-col gap-1.5 w-full">
      ...
      <div className="relative">
        <input
          ...
          className={cn(
            'w-full h-12 rounded-xl border bg-white px-4 pr-11 text-sm text-slate-900 placeholder:text-slate-400',
    ...
    <div className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 group">
      {editing ? (
        <input
          ...
          onBlur={commit}
          onKeyDown={handleKey}
          className="flex-1 min-w-0 text-sm bg-white border border-blue-400 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
        />
      ) : (
      ...
      <div className="flex items-center gap-1 shrink-0">
        {editing ? (
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-blue-600 bg-blue-50 opacity-0 group-hover:opacity-100 hover:bg-blue-100 transition-all"
          >
            <Edit2 size={13} />
          </button>
        )}
        <button
          ...
          className="p-1.5 rounded-lg text-red-500 bg-red-50 opacity-0 group-hover:opacity-100 hover:bg-red-100 transition-all"
        >
          <X size={13} />
        </button>
      </div>
...
const AdminSettingsPage: React.FC = () => {
  ...
  const {
    register: regPlatform,
    handleSubmit: handlePlatform,
    setValue: setPlatformValue,
    formState: { errors: platformErrors },
  } = useForm<PlatformValues>({
    resolver: zodResolver(platformSchema),
    defaultValues: {
      siteName: 'InnovateGuide',
      whatsappNumber: '',
    },
  })
  ...
  const onSavePlatform = async (data: PlatformValues) => {
    ...
    try {
      // Persist to localStorage (replace with real API call in production)
  ...
  const onChangePassword = async (_data: PasswordValues) => {
    ...
    try {
      // Replace with real authService.changePassword() call
  ...
  const saveCategories = async () => {
    ...
    try {
      ...
      toast.success(`${categories.length} categories saved successfully.`)
    } catch {
  ...
  const handleNewCatKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminHeader
        title="Settings"
        subtitle="Configure platform behaviour and security"
      />
      ...
      <div className="max-w-3xl mx-auto w-full space-y-6 pb-10">
        ...
        <Section
          ...
          subtitle="Site name and WhatsApp contact number"
        >
          <form onSubmit={handlePlatform(onSavePlatform)} className="space-y-5" noValidate>
            ...
            <Input
              label="Site Name"
              {...regPlatform('siteName')}
              placeholder="e.g. InnovateGuide"
              error={platformErrors.siteName?.message}
            />

            {/* WhatsApp Number */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
                <MessageCircle size={13} className="inline mr-1.5 text-green-600" />
                WhatsApp Number
              </label>
              <Input
                {...regPlatform('whatsappNumber')}
                placeholder="919876543210 (country code + number, no spaces or +)"
                error={platformErrors.whatsappNumber?.message}
              />
              <p className="text-xs text-slate-400 mt-1.5">
                Used for the "Buy on WhatsApp" CTA on project detail pages.
                Example: <code className="bg-slate-100 px-1 rounded">919876543210</code> for +91 98765 43210.
              </p>
            </div>
        ...
        >
          <form onSubmit={handlePassword(onChangePassword)} className="space-y-5" noValidate>
            ...
            <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 text-xs text-amber-800 leading-relaxed">
              <strong>Note:</strong> In production this updates the admin credentials via the
              secure API endpoint. Make sure your new password is at least 8 characters, contains
        ...
        <Section
          ...
          subtitle="Manage the domain categories shown in browse and filter panels"
        >
          {/* Add new category */}
          <div className="flex gap-2 mb-4">
            <input
              ...
              className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all"
            />
            <button
              type="button"
              onClick={addCategory}
              className="h-10 px-3.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 transition-colors border border-emerald-200 shrink-0"
            >
              <Plus size={16} />
            </button>
          </div>
          ...
          <p className="text-xs text-slate-400 mb-4">
            {categories.length} categor{categories.length !== 1 ? 'ies' : 'y'} &mdash;
            hover a row to reveal edit / delete actions.
          </p>
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-700 transition-colors px-3 h-8 rounded-lg hover:bg-slate-100"
            >
              <RefreshCw size={13} />
              Reset to defaults
            </button>
