...
const MOCK_REQUESTS: ProjectRequest[] = [
  {
    ...
    description:
      'I need a plant disease detection system using CNN that can identify common leaf diseases from uploaded images. The app should have a React frontend where users can upload images and get predictions with confidence scores.',
    additionalInfo:
      'Deadline is before my university viva on 30 June. Must include a detailed project report and presentation slides.',
    status: 'pending',
  ...
  },
  {
    ...
    description:
      'A college event management portal where students can register for events, pay entry fees online, and receive e-tickets via email. Admin should be able to add events, view registrations, and export attendance sheets.',
    additionalInfo:
      'The college logo and colour scheme (navy blue and gold) must be used. Source code ownership is required.',
    status: 'in-progress',
  ...
  },
  {
    ...
    description:
      'Smart greenhouse monitoring system with temperature, humidity, and soil moisture sensors. Data should be pushed to a cloud dashboard in real time and trigger alerts via SMS when thresholds are crossed.',
    additionalInfo:
      'I already have the hardware. I only need the firmware and the dashboard integration code.',
    status: 'completed',
  ...
  },
  {
    ...
    description:
      'A decentralised academic credential verification system where universities can issue digitally signed certificates as NFTs. Employers can verify authenticity on-chain without contacting the institution.',
    additionalInfo:
      'This is for my MTech thesis. I need full documentation, UML diagrams, and a demo video for submission.',
    status: 'pending',
  ...
  },
  {
    id: 'req-005',
    name: 'Mrunal Desai',
    email: 'mrunal.desai@hotmail.com',
    ...
    description:
      'A daily habit tracker mobile app with streak counters, motivational quotes, push reminders, and a weekly progress chart. Users should be able to create custom habits and set reminder times.',
    additionalInfo:
      'Must support both Android and iOS. Firebase Authentication for login and Firestore for data storage is preferred.',
    status: 'rejected',
...
const ProjectRequestsPage: React.FC = () => {
  ...
  const openViewDialog = (req: ProjectRequest) => {
    setViewTarget(req)
  ...
  const saveStatusChange = () => {
    ...
    setTimeout(() => {
      setRequests((prev) =>
        prev.map((r) =>
          r.id === viewTarget.id ? { ...r, status: statusUpdate } : r
        )
      )
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminHeader title="Project Requests" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm font-inter">
            ...
            <tbody className="divide-y divide-border">
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-16 text-muted font-inter text-sm">
                    No requests found.
                  </td>
                </tr>
              ) : (
                paginated.map((req) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <Badge variant="blue">{req.budget}</Badge>
                    </td>
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <Badge variant="default">{req.projectType}</Badge>
                    </td>
                    <td className="px-5 py-3.5 whitespace-nowrap">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex justify-center">
                        <button
                          ...
                          className="
                            inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-inter font-medium
                            rounded-lg bg-teal-50 text-teal-700 hover:bg-teal-100
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-5 py-3 bg-white">
            <p className="text-xs font-inter text-muted">
              Page {currentPage} of {totalPages} &mdash; {requests.length} total
            </p>
      ...
      <Dialog open={!!viewTarget} onOpenChange={() => setViewTarget(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Full information for this custom project request
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-5 text-sm font-inter">
              ...
              <Section title="Technologies">
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {viewTarget.technologies.map((tech) => (
              ...
              <Section title="Update Status">
                <div className="flex items-center gap-3 mt-1">
                  <select
                    ...
                    className="
                      flex-1 px-3 py-2 text-sm font-inter
                  ...
                  <Button
                    ...
                    disabled={statusUpdate === viewTarget.status || statusSaving}
                  >
                    Save
                  </Button>
                </div>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({
  ...
  <div className="space-y-2">
    <h3 className="text-xs font-semibold font-inter uppercase tracking-wider text-muted border-b border-border pb-1">
      {title}
    </h3>
    {children}
  </div>
